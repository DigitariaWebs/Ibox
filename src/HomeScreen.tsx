      ...
      <View style={{ flex: 1, paddingHorizontal: 24, paddingVertical: 32 }}>
        ...
        <View style={{ backgroundColor: '#F3F4F6', borderRadius: 12, padding: 24, alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', gap: 12, marginBottom: 16 }}>
            <Button title={t('increment')} onPress={() => dispatch(increment())} variant="primary" size="sm" />
            <Button title={t('decrement')} onPress={() => dispatch(decrement())} variant="secondary" size="sm" />
          </View>
          <Button title={t('increment_by_amount')} onPress={() => dispatch(incrementByAmount(5))} variant="outline" size="sm" />
        </View>
      </View>
    </ScrollView>
  );
